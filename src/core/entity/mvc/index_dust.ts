...
//   }) {
//     const walletAddress = mvc.Address.fromString(this.connector.address, 'testnet' as any)
...
//     const dusts = await fetchUtxos({ address: protocolAddress, network: 'testnet' })
//     if (dusts.length > 0) {
...
//       // if so, use it directly;
//       const dusts = await fetchUtxos({ address: parent.address, network: 'testnet' })
//       if (dusts.length > 0) {
//         dustTxid = dusts[0].txid
//         dustValue = dusts[0].value
//       } else {
...
//     // 1.1 first, check if root address already has dust utxos;
//     // if so, use it directly;
//     const dusts = await fetchUtxos({ address: root.address, network: 'testnet' })
//     if (dusts.length > 0) {
//       dustTxid = dusts[0].txid
//       dustValue = dusts[0].value
//     } else {
...
//       const isValid = !!(await fetchTxid({ txid, network: 'testnet' }))
//       if (isValid) {
