...
export class Psbt {
  ...
  finalizeInput(inputIndex: number, finalScriptsFunc?: FinalScriptsFunc | FinalTaprootScriptsFunc): this {
    ...
    if (isTaprootInput(input))
      return this._finalizeTaprootInput(inputIndex, input, undefined, finalScriptsFunc as FinalTaprootScriptsFunc)
  ...
  private _validateSignaturesOfInput(inputIndex: number, validator: ValidateSigFunction, pubkey?: Buffer): boolean {
    const input = this.data.inputs[inputIndex]
  ...
  signInputHD(inputIndex: number, hdKeyPair: HDSigner, sighashTypes: number[] = [Transaction.SIGHASH_ALL]): this {
    if (!hdKeyPair || !hdKeyPair.publicKey || !hdKeyPair.fingerprint) {
      throw new Error('Need HDSigner to sign input')
    }
  ...
  private _signInput(inputIndex: number, keyPair: Signer, sighashTypes: number[] = [Transaction.SIGHASH_ALL]): this {
  ...
  private async _signTaprootInputAsync(
    ...
    const signaturePromises: Promise<{ tapKeySig: Buffer } | { tapScriptSig: TapScriptSig[] }>[] = []
 ...
 */
const transactionFromBuffer: TransactionFromBuffer = (buffer: Buffer): ITransaction => new PsbtTransaction(buffer)
...
function scriptCheckerFactory(
  ...
  return (inputIndex: number, scriptPubKey: Buffer, redeemScript: Buffer, ioType: 'input' | 'output'): void => {
...
function getTxCacheValue(key: TxCacheNumberKey, name: string, inputs: PsbtInput[], c: PsbtCache): number | undefined {
...
function getTaprootHashesForSig(
  ...
  const prevOuts: Output[] = inputs.map((i, index) => getScriptAndAmountFromUtxo(index, i, cache))
...
function getPayment(script: Buffer, scriptType: string, partialSig: PartialSig[]): payments.Payment {
...
function getScriptFromInput(inputIndex: number, input: PsbtInput, cache: PsbtCache): GetScriptReturn {
  const unsignedTx = cache.__TX
...
function inputFinalizeGetAmts(inputs: PsbtInput[], tx: Transaction, cache: PsbtCache, mustFinalize: boolean): void {
  let inputAmount = 0
...
function nonWitnessUtxoTxFromCache(cache: PsbtCache, input: PsbtInput, inputIndex: number): Transaction {
...
function pubkeyInInput(pubkey: Buffer, input: PsbtInput, inputIndex: number, cache: PsbtCache): boolean {
...
function pubkeyInOutput(pubkey: Buffer, output: PsbtOutput, outputIndex: number, cache: PsbtCache): boolean {
