...
export function signBtc(
  ...
  network = network || bitcoin.networks.bitcoin
  if (utxoTx.memo) {
    const buf = base.isHexString(utxoTx.memo) ? base.fromHex(utxoTx.memo) : Buffer.from(base.toUtf8(utxoTx.memo))
    if (buf.length > 80) {
...
export function estimateBchFee(utxoTx: utxoTx, network?: bitcoin.Network) {
  ...
  const { virtualSize } = calculateBchTxSize(inputs, outputs, utxoTx.address, fakePrivateKey, network, dustSize)
  return virtualSize * feePerB
}
